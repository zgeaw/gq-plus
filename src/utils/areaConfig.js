...
export default {
  ...
  searchArea (str) {
    let list = [];
    let getName = (array = [], str) => {
      let newItem = {};
      array.map((item, i) => {
        if (item.name.indexOf(str) >= 0) {
          newItem = {
            code: item.adcode,
            text: this.getTextByCode(item.adcode).replace(str, '<span class="hotSearchKey">' + str + '</span>')
          }
          list.push(newItem);
        } else {
          ...
          getName(item.districts, str)
        }
      })
    }
    getName(areaData, str)
    let newList = list.filter(item => item.text.indexOf(str) >= 0);
    return newList
  }
}